...
interface IGroupingScope extends IScope {
    vm: IViewModel;
    structure: Structure;
}
...
class Controller implements ng.IController, IViewModel {
    ...
    constructor(private $scope: IGroupingScope,
    ...
    createGrouping = async (name: string): Promise<void> => {
        await this.groupingService.createGrouping(vieScolaire.structure.id, name)
            .then(res => {
                let grouping: Grouping = new Grouping().construct(res.data.id, name, vieScolaire.structure.id, []);
    ...
    deleteGroupingAudience = async (grouping: Grouping, studentDivision: Classe): Promise<void> => {
        try {
            ...
            grouping.student_divisions = grouping.student_divisions.filter((studentDivisionFilter: Student_division) => studentDivisionFilter.id != studentDivision.id);
        } catch (e) {
