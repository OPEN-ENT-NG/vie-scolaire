...
import fr.openent.evaluations.service.NiveauEnseignementComplementService;
...
public class DefaultNiveauEnseignementComplementService extends SqlCrudService implements NiveauEnseignementComplementService {
    ...
    public void updateEnsCpl(Integer id, JsonObject data, Handler<Either<String, JsonObject>> handler) {
        ...
        String query = "UPDATE "+ Viescolaire.EVAL_SCHEMA+".eleve_enseignement_complement SET id_enscpl = ?, niveau = ?"
             +" WHERE id = ?";
    ...
    public void getNiveauEnsCplByEleve(String idEleve, Handler<Either<String, JsonObject>> handler) {
        ...
        String query ="SELECT * FROM "+Viescolaire.EVAL_SCHEMA+".eleve_enseignement_complement WHERE id_eleve = ?";
