...
public class DefaultGroupeService extends SqlCrudService implements GroupeService {
    ...
    public void search(String structureId, String userId, String query, List<String> fields, Handler<Either<String, JsonArray>> handler) {
        ...
        String queryFromUserId = "MATCH (u:User {id:{userId}})-[:IN]->" +
                ...
                getQueryFilter(fields, params) + "RETURN g.id as id, g.name as name ORDER BY g.name " +
                "UNION " +
                ...
                getQueryFilter(fields, params) + "RETURN DISTINCT g.id as id, g.name as name ORDER BY g.name";
    ...
    private StringBuilder getQueryFilter(List<String> fields, JsonObject params) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            filter.append("toLower(g.{field").append(i).append("}) CONTAINS {query} ");
            params.put(Field.FIELDS + i, field);
        }
