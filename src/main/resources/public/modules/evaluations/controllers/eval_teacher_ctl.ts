            ...
            editDevoir : function (params) {
                ...
                $scope.devoir.competences.sync().then(() => {
                    ...
                    for (var i=0; i < parentToCheck.length; i++){
                        var checkIt = true;
                        for (var j=0; j < $scope.competencesFilter.length; j++){
                            for(var k=0; k < $scope.evaluations.enseignements.all.length; k++){
                                var currComp =$scope.competencesFilter[
                                    $scope.evaluations.competencesDevoir[j].id_competence+'_'
                                + $scope.evaluations.enseignements.all[k].id];

                                if( currComp!== undefined && currComp.id_parent === parentToCheck[i].id){
                                    checkIt = currComp.isSelected;
                                }
                                // si on rencontre un fils non selectionné on arrête de chercher
                                if(!checkIt) {
                                    j = $scope.competencesFilter.length;
                                    k = $scope.evaluations.enseignements.all.length;
                                }
                            }
                        }
                        if(checkIt){
